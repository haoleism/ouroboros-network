...
data NodeCallbacks m blk hdr = NodeCallbacks {
      -- | Produce a block
...
data InternalState m up blk hdr = IS {
    ...
    , callbacks     :: NodeCallbacks m blk hdr
...
initInternalState :: forall m up blk hdr.
                  ...
                  -> ChainDB m blk hdr (ExtLedgerState blk)
                  -> NodeCallbacks m blk hdr        -- ^ Callbacks
        ...
        case mIsLeader of
          ...
          Just proof -> do
            ...
            newBlock <- runProtocol varDRG $
              produceBlock proof l currentSlot (Chain.castPoint prevPoint) prevNo
            return $ Just newBlock

      whenJust mNewBlock $ \newBlock ->
        ChainDB.addBlock chainDB (Chain.blockPoint newBlock) newBlock
  where
...
initNetworkLayer NetworkRequires{..} = do
    ...
    runInBg $ blockFetchLogic nrBlockFetch fetchClientRegistry
